...
# MIN_FILES — нижняя граница вменяемости списка, а не цель: сейчас файлов ~85,
# и упасть до десятка список может только если каталоги переехали.
MIN_FILES=20
...
echo "Проверено файлов: $N_FILES"
echo

# Урок из шапки этого файла: зелёная галочка, полученная на пустом списке,
# опаснее отсутствующей. Переедет docs/core или .claude/agents — find уйдёт
# в /dev/null, список схлопнется, и «привязок нет» будет означать «ничего не
# смотрели». Поэтому число проверенных файлов печатается всегда, а слишком
# короткий список — отказ, а не успех.
if [ "$N_FILES" -lt "$MIN_FILES" ]; then
  echo "✗ ПРОВЕРКА НЕ СОСТОЯЛАСЬ: файлов $N_FILES, ожидалось не меньше $MIN_FILES."
  echo "  Универсальная база не найдена по ожидаемым путям — скорее всего каталоги"
  echo "  переехали. Почини пути сбора FILES, а не порог."
  exit 1
fi
