...
- `docs/core/design-principles.md` — раздел «Работа с референсами»: как разбирать визуальную референцию в значения, что считается доминирующим паттерном, почему не усреднять.
- `docs/projects/<проект>/brand.md` — тон и характер активного проекта. Референсы должны отражать именно его вкус, а не общий «красивый дизайн».
- `docs/decisions/ADR-007-reference-sources-policy.md` — какие источники доступны (clean vs. local-only), где риски.
...
6. **Dials** (см. `docs/core/design-principles.md`):
