...
Этот файл живёт в двух раскладках — внутри пакета Nullume и в репозитории, куда
его поставили, а каталоги документации там разные. Поэтому ищи по имени файла,
а не по пути: `Glob` для этого и нужен.

- **`ADR-007-reference-sources-policy.md`** — политика источников: что можно
  везде (clean), что только локально (local-only), где риски. Читать до первого
  импорта. Не нашёл файл — скажи об этом и не трогай local-only источники.
- **`brand.md` активного проекта** — бренд, тон и характер. Референсы должны
  отражать именно его вкус, а не общий «красивый дизайн». Активный проект
  назван в файле `.active` рядом с каталогами проектов.
...
Результат — набор картинок с метаданными.

**Смотреть глазами обязательно.** У каждого референса есть `previewPath` —
откройте эти файлы инструментом `Read` и посмотрите на изображения. Описывать
семейство по тегам, названию файла или палитре, не открыв картинки, запрещено:
так рождаются дескрипторы, не имеющие отношения к тому, что на снимке.
Просмотрите первые 10–15 результатов и отметьте устойчивые паттерны.
...
слово: exemplar — это референс, **ближайший к центру кластера по эмбеддингу**,
то есть самый типичный для группы, а не самый удачный. Машина знает только
«похоже» и «типично»; «лучше» знаете вы.

Откройте `previewPath` каждого exemplar через `Read` и посмотрите: что общего,
чем отличаются, какой референс в группе лишний.

### На что опираться, решая, какой референс брать

Порядок именно такой, сверху вниз:

1. **Бренд проекта** (`brand.md` активного проекта, см. пред-чтение выше) —
   тон, характер, аудитория. Референс, красивый сам по себе, но чужой бренду,
   отбраковывается первым.
2. **Регуляторы (dials) проекта** — вариативность, плотность, декор, движение.
   Референс, выбивающийся из заданных значений, тянет семейство не туда.
3. **Доменный профиль** (`docs/core/design-profiles.md`) — норма для SaaS может
   быть дефектом для магазина и наоборот.
4. **Глобальные запреты** (`docs/core/design-principles.md`) — фиолетово-синий
   градиент, системный шрифт как бренд-шрифт, серый текст на цветном,
   вложенные карточки, bounce-анимация. Такой референс не берётся вовсе.
5. **Внутренняя однородность семейства** — если в группе два языка вёрстки,
   это два семейства, а не одно. Не усредняйте: каждая линия ведётся до конца.

Отбракованное фиксируйте: поле `divergences` дескриптора — про то, чем
семейство расходится со своими же образцами, `antiRefCheck` — про то, какие
запреты вы проверили на этом семействе.
