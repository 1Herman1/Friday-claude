...
**профиль Симбы здесь неприменим**: у Симбы вход держится на токене в
заголовке, здесь — на куке. Поэтому класс CSRF, который там почти неприменим,
здесь живой (см. ниже).
...
|---|---|---|---|
| `ps_sid` | UUID **гостевой сессии корзины** (`randomUUID()`), подписанная кука (`signed: true`, секрет `PS_COOKIE_SECRET`). Это НЕ сессия входа | `server/src/routes/cart/index.ts` (`POST /api/v1/cart/items`, когда владелец не определён) | `httpOnly: true`, `sameSite: 'lax'`, `secure` при любой среде кроме разработки и тестов, `path: '/'`, `maxAge` 180 дней |
| `ps_auth` | **JWT входа** (payload `{ userId, role, tv }`, HS256, `expiresIn: '7 days'`) | `server/src/routes/auth/verify-otp.ts` | `httpOnly: true`, `sameSite: 'lax'`, `secure` при любой среде кроме разработки и тестов, `path: '/'`, `maxAge` 7 дней. Снимается в `routes/auth/logout.ts` |

`verify-otp` и ставит куку `ps_auth`, и возвращает `token` в теле ответа.
**Браузерный клиент токен не хранит** и заголовок `Authorization` не шлёт:
вход держится на куке, которую браузер прикладывает сам благодаря
`credentials: 'include'` (`client/src/lib/api.ts`). Токен в теле остаётся для
клиентов, которые не браузер, — им пользуются интеграционные тесты.

Отсюда следует условие на размещение API — см. одноимённый раздел ниже.
...
| Утечка токена через XSS | JWT лежит только в httpOnly-куке `ps_auth`; клиент его не хранит | Появление `localStorage.setItem` с токеном — это возврат уязвимости, а не оптимизация |
...
**Массовые 401 больше не означают недоступную базу.** Раньше обращение к базе
стояло внутри того же `try`, что и разбор токена, и любая её ошибка
превращалась в «недействительный токен»: упавшая база выбрасывала из
аккаунтов всех разом, а в мониторинге читалась как массовая проблема с
авторизацией — диагностика уводилась в сторону в самый неподходящий момент.
Теперь в 401 превращаются только ошибки самого токена, остальное уходит в 500
и пишется в лог как есть.

**Строка `optional auth: credentials present but rejected`** в логах означает,
что пользователь пришёл с токеном, токен не подошёл, и дальше он работает как
гость. Массовое появление — признак либо истёкших сессий (норма после смены
`JWT_SECRET`), либо отзыва токенов, либо подбора.

## Определение среды — одно на весь сервер

`server/src/lib/env.ts` — **единственное место**, где читается `NODE_ENV`.
Появление `process.env.NODE_ENV` где-либо ещё в `server/src` — находка:
развилка, которая снова разойдётся.

Правило: **неизвестное значение считается боевым.** `isDevelopment` истинно
только для `development` и `test`, всё прочее — прод.

Так сделано потому, что раньше сравнения разошлись по направлению. Почта и SMS
при незнакомом значении брали боевой отправщик (безопасно), а куки и проверка
секретов считали окружение разработкой — то есть фейлились в опасную сторону
ровно те три места, которые за безопасность и отвечают. При `NODE_ENV=prod`
сервер поднимался с дефолтными секретами и раздавал куки без признака
защищённого соединения.

Проверять при ревью: `grep -rn NODE_ENV server/src` — вхождений быть должно
ровно одно.

## Условие на размещение API

Вход в браузере держится **только на куке** `ps_auth`, а у неё
`sameSite: 'lax'`. Отсюда жёсткое требование к будущей выкатке:

> **API обязан жить на том же домене второго уровня, что и сайт.**
> `shop.example.com` + `api.example.com` — работает.
> `example.com` + `api-example.net` — **не работает**: браузер не приложит
> `lax`-куку к запросу на чужой домен.

Симптом при нарушении коварный: сетевых ошибок нет, `CORS` пропускает, но
сервер видит гостя и отвечает `401`. Выглядит как «сломался вход», а причина —
в выборе домена.

Токен по-прежнему возвращается телом ответа `verify-otp` — это законный путь
для клиента, который не браузер (тесты, скрипты, мобильное приложение). Именно
**хранить** его в браузере нельзя: тогда его достаёт любой XSS. Передача
одноразово по HTTPS такой проблемы не создаёт.

Понадобится API на чужом домене — это не правка одной строки: `sameSite`
придётся ставить в `none`, а значит обзаводиться защитой от подделки запроса
(§6), которой сейчас нет.

## Известные и принятые риски

| Риск | Почему принят | Пересмотреть |
|---|---|---|
| Вход в браузере держится только на куке `ps_auth` с `sameSite: 'lax'` | Клиент больше не хранит токен: копия в `localStorage` обнуляла бы смысл `httpOnly`. Плата за это — условие на топологию выкатки, см. раздел ниже | Если API окажется на другом домене второго уровня, чем сайт |
| Необязательная авторизация продолжает как гость при протухшем токене | Так и задумано: корзину должно быть видно всем | Теперь это пишется в лог предупреждением (`plugins/authenticate.ts`) — «пропала корзина» стало возможно разобрать |
