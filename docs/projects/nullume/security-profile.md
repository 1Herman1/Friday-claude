...
| §3 Доступ без проверки | —; нет многопользовательского доступа; Гермес один на машине |
...
Риск: Pinterest/Dribbble/X API ToS может запретить автоматизацию; владелец (Гермес) принимает риск блокировки.
...
- Утечка файла `config.json` → пересоздание, так как только Гермес имеет доступ к `~/.nullume/`
...
|---|---|---|
| Pinterest local-only под собственной сессией Гермеса | Нужна Pinterest для стилей; официально это нарушает ToS | Если счёт заблокируют — перейти на платный X API или отказ от Pinterest |
...
| Ключи на диске с правами 600 | Защита от других пользователей достаточна; Гермес один на машине | Если работают много пользователей — перейти на системный keychain (macOS) или libsecret (Linux) |
