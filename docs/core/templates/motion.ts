/**
 * ШАБЛОН. Копируется в проект при подключении движка анимации, путь назначения
 * фиксируется в `project.md` проекта. Сам по себе этот файл не собирается и ни
 * в один бандл не входит — он эталон, а не рабочий модуль.
 *
 * Единая точка входа в анимацию. Прямые импорты из `animejs` в компонентах
 * запрещены: библиотека не знает про `prefers-reduced-motion`, и любой обход
 * этого модуля нарушает правило из `docs/core/motion.md`. Соответствие доктрины
 * примитивам — `docs/core/motion-engine.md`.
 *
 * Требует `animejs` в зависимостях проекта:  npm i animejs
 */
